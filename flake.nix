      ...
      in {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            ${if isDarwin then ''
              ZBAR_LIB_PATH="${pkgs.zbar.lib}/lib/libzbar.dylib"
            '' else ''
              ZBAR_LIB_PATH="${pkgs.zbar.lib}/lib/libzbar.so"
            ''}
            ...
            if [ -n "$VENV_PATH" ]; then
              ...
              if [ -n "$PYZBAR_LIB" ]; then
                sed -i "s|path = find_library('zbar')|path = '$ZBAR_LIB_PATH'|g" "$PYZBAR_LIB"
                sed -i "s|path = '/nix/store/[^']*libzbar\.[^']*'|path = '$ZBAR_LIB_PATH'|g" "$PYZBAR_LIB"
              fi
            fi
            ...
            if [ -n "$VENV_PATH" ] && [ -f "$VENV_PATH/bin/activate" ]; then
              ...
              ''}
              ${pkgs.lib.optionalString isDarwin ''
                echo "export DYLD_LIBRARY_PATH="$DYLD_LIBRARY_PATH" # nix-ld-inject" >> "$VENV_PATH/bin/activate"
              ''}
            fi
